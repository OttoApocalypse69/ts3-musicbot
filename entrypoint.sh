...
# Set correct HOME path explicitly
export HOME=/home/ts3bot
...
if [ -z "$DBUS_SESSION_BUS_ADDRESS" ]; then
    ...
    eval $(su -s /bin/bash -c "dbus-launch --sh-syntax" ts3bot)
fi

# Ensure PulseAudio is running as ts3bot and virtual devices are initialized
echo "Starting PulseAudio daemon..."
su -s /bin/bash -c "pulseaudio --start --exit-idle-time=-1 --system=false" ts3bot

# Wait for PulseAudio to start up and register
for i in {1..10}; do
    if su -s /bin/bash -c "pactl info" ts3bot >/dev/null 2>&1; then
...
su -s /bin/bash -c "pactl set-default-sink VirtualSink" ts3bot || true
su -s /bin/bash -c "pactl set-default-source VirtualSink.monitor" ts3bot || true
...
# Ensure permissions and ownership are correct on the generated config files
chmod 600 "$CONFIG_FILE" "$COMMAND_CONFIG_FILE"
chown ts3bot:ts3bot "$CONFIG_FILE" "$COMMAND_CONFIG_FILE"
...
# Forward all arguments including the command configuration file
exec su -s /bin/bash -c "exec python3 /app/web_ui.py --config \"$CONFIG_FILE\" --command-config \"$COMMAND_CONFIG_FILE\" \"$@\"" ts3bot
